...
def scrap_folder(folder_path: str):
    ...
    """
    onlyfiles = [
        f for f in tqdm(listdir(folder_path)) if isfile(join(folder_path, f))
    ]
    all_dfs = []
...
def clean_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "DATE LIBELLE" in df_final.columns:
        ...
        if ("CREDIT" not in df_final.columns) and (
            "DEBIT" not in df_final.columns
        ):
            df_final.rename(
                columns={"x_2": "DEBIT", "x_3": "CREDIT"}, inplace=True
            )
        ...
        df_final["CREDIT"] = convert_to_numeric_with_cleanup(
            df_final["CREDIT"]
        )
